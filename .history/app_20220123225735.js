...
document.body.addEventListener("click", function (e) {
  const arr=document.querySelectorAll('.modal-content img');
  const lightboxx=document.querySelector('.modal-content img');
  let lightSrc="";
  console.log(lightboxx);
  if (e.target.matches(".modal")) {
   ...
   lightSrc=lightboxx.getAttribute("src");
  
   index=[...lightbox].findIndex(item=>item.dataset.img===lightSrc);
  index=index+1;
  console.log(index);
  // const newSrc=[...lightbox][index].getAttribute("src");
  // lightboxx.setAttribute("src",newSrc);
  lightboxx.setAttribute("src",)
  }
});
